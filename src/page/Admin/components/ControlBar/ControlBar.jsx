...
const ControlBar = ({ onReload, onSearch, onSave }) => {
  return (
    <div className="flex w-full justify-between">
      <Button variant="contained" color="black" startIcon={<MdAutorenew />} onClick={onReload}>
        새로고침
      </Button>
      <div className="flex w-1/2 justify-center">
        <Input type="search" className="mr-3 w-2/3" placeholder="이름을 입력해주세요" color="yellowPoint" />
        <Button className="w-1/6" variant="contained" color="yellowPoint" startIcon={<CiSearch />} onClick={onSearch}>
          검색
        </Button>
      </div>
